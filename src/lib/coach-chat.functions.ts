...
export const chatWithCoach = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [onb, program, recent, sources, history] = await Promise.all([
      ...
      supabase.from("workout_sessions").select("date, difficulty_rating, energy_rating, performance_rating, soreness_rating, logged_sets(weight, reps, rir, program_exercises(exercise_name))").eq("user_id", userId).eq("completed", true).order("date", { ascending: false }).limit(3),
    ...
    // Cite up to 2 knowledge sources that were surfaced to the model
    const citations = (sources.data ?? []).slice(0, 2).map((s) => ({ source: [s.title, s.author].filter(Boolean).join(" — ") }));

    await supabase.from("ai_messages").insert({
      conversation_id: conversationId, role: "assistant", content: reply, citations,
    });
...
function buildSystemPrompt(ctx: {
  ...
  program: {
    ...
    program_days: Array<{ name: string; muscle_groups: string[]; program_exercises: Array<{ exercise_name: string; sets: number; rep_range: string; target_rir: number | null }> }>;
  } | null;
  recentSessions: Array<{ date: string; difficulty_rating: number | null; energy_rating: number | null; performance_rating: number | null; soreness_rating: number | null; logged_sets: Array<{ weight: number | null; reps: number | null; rir: number | null; program_exercises: { exercise_name: string } | null }> }> | null;
  ...
  if (ctx.recentSessions?.length) {
    ...
    ctx.recentSessions.forEach((s) => {
      const lifts = s.logged_sets.slice(0, 6).map((l) => `${l.program_exercises?.exercise_name}: ${l.weight}kg x${l.reps} @RIR${l.rir ?? "-"}`).join("; ");
      parts.push(`- ${s.date} · diff ${s.difficulty_rating}/5 · energy ${s.energy_rating}/5 · perf ${s.performance_rating}/5 · sore ${s.soreness_rating}/5 · ${lifts}`);
    });
  }
